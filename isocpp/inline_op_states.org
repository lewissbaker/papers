...
<ins>  struct <expos>default-state-impls</expos> {
  ...
  template&lt;class Tag&gt;
  struct <expos><ins>sender-</ins>impls-for</expos><del> : <expos>default-impls</expos> {}</del>;       <expos>// exposition only</expos>
...
<p class="numbered">
The expression in the <code>noexcept clause of the <code>connect</code> member function of <code><expos>basic-sender</expos></code> is:
<pre>
