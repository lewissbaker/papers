...
Insert the following paragraph between [exec.let] p2 and p3:

#+BEGIN_EXPORT html
<div class="wording">
<ins>The exposition-only class template
<expos>sender-impls-for</expos> (33.9.1) is specialized for <expos>let-cpo</expos> as follows:</ins>

<pre><ins>template&lt;&gt;
struct <expos>sender-impls-for</expos>&lt;<expos>let-cpo</expos>&gt; : <expos>default-sender-impls</expos> {
