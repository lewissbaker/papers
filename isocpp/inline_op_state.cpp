...
template <typename T>
concept _tuple_like = requires { typename std::tuple_size<T>; };
...
concept _has_completion_signatures_member_type =
    requires { typename remove_cvref_t<Sndr>::completion_signatures; };
...
protected:
  ...
  using child_t = _manual_child_operation<
      ParentOp,
      _indexed_tag<Id>,
...
private:
  ...
  struct _construct_helper {
    ...
    _construct_helper(
        _manual_child_operations& self, bool& succeeded, Child...[Id] && child)
      : self(self)
      , succeeded(succeeded) {
...
public:
  void _construct(Child&&... child) noexcept(
      (child_t<Ids>::_is_nothrow_connectable && ...)) {
    bool succeeded = false;
    ((_construct_helper<Ids>{*this, succeeded, std::forward<Child>(child)},
      ...),
     succeeded = true);
  }

  void _destruct() noexcept { (child_t<Ids>::_destruct(), ...); }
...
public:
  ...
  ~_basic_operation() { children_t::_destruct(); }

  void start() noexcept { this->_start(); }
};
...
struct _default_sender_impls {
  ...
  template <typename Tag, typename Data>
  static _default_state<Tag, std::decay_t<Data>>
  _get_state(Tag, Data&& data) noexcept {
...
template <typename Tag, typename Data>
using _state_type_t =
    decltype(_sender_impls_for<Tag>::_get_state(Tag{}, std::declval<Data>()));
...
struct _basic_sender {
  ...
  _basic_operation<
      Tag,
      Rcvr,
      _state_type_t<Tag, _member_t<Self, Data>>,
...
struct then_t {
  ...
  static _basic_sender<then_t, std::decay_t<Func>, std::remove_cvref_t<Source>>
  operator()(Source&& src, Func&& func) {
  ...
  void _complete(
    ...
    if constexpr (same_as<CompletionTag, set_value_t>) {
      ...
      constexpr bool is_nothrow =
          std::is_nothrow_invocable_v<std::invoke_result_t<Func, Datums...>>;
      try {
        if constexpr (std::is_void_v<result_t>) {
          std::invoke(
              std::forward<Func>(self.data), std::forward<Datums>(datums)...);
          execution::set_value(std::move(self.get_receiver()));
        } else {
          execution::set_value(
              std::move(self.get_receiver()),
              std::invoke(
                  std::forward<Func>(self.data),
...
struct _state_impls_for<just_t> : _default_state_impls {
  ...
  void _start(this Self& self) noexcept {
    std::apply(
        [&](auto&&... vs) noexcept {
          execution::set_value(
              std::move(self.get_receiver()), static_cast<decltype(vs)>(vs)...);
        },
        std::move(self.data));
  }
};
...
int main() {
  ...
  for (int i = 0; i < iters; ++i) {
    auto result =
        stdex::sync_wait(make_sender(loop, 6, 5, 2, 1), ss.get_token());
    if (!result.has_value()) {
      std::terminate();
    }
  }
