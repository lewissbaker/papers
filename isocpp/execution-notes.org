...
- ~env<Envs...>~ class contains a ~query(QueryTag)~ member-function that only takes the query-tag.
  ...
  to ~env<Envs...>~ and ~prop<Query, Value>~ and all of the other places where we define a ~query()~
  function.
...
- ~execution::transform_sender~ can return dangling references if a custom domain returns a prvalue.
  ...
  #+end_src
  In this example, the implementation of ~std::execution::transform_sender~ is required to have
