...
2. Enable the use of exception-based error handling in freestanding environments
   by enabling 
3. Eliminate classes of bugs Make it possible to identify at compile-time, code that is failing to
...
#+begin_src c++
void f() throw();     // empty static exception specification - equivalent to noexcept
void g() throw(A, B); // static exception specification
void h() throw(...);  // dynamic exception specification - equivalent to noexcept(false)
#+end_src
...
Example 3: Querying the potentially-thrown exception types + template catch
#+begin_src c++f
std::variant<std::monostate, declthrow(g())...> err; // variant<monostate, A, B>
try {
  g(); // might throw A or B
} template catch (auto e) { // instantiated for each static exception type
...
void nothrow_caller() throw() {
  f(); // OK
  g(); // Error: A and B not handled locally and not listed in throw specification
  
  try {
    g(); // Error: B not handled locally and not listed in throw specification
  } catch (A) {}
  
  try {
    g(); // OK: all potentially-thrown exceptions are handled locally
  } catch (A) {
  } catch (B) {
  }
  
  h(); // Error: dynamic exception not handled locally
...
void caller() throw(A) {
  ...
  try {
    ...
    throw A{a}; // OK - only throws A - slices types derived from A
  } catch(...) {
  }
}
#+end_src

Example 6: Simplifies std::expected/std::variant code
#+begin_src c++
std::expected<X, E1> foo() noexcept;
std::expected<Y, E2> bar() noexcept;

std::expected<Z, std::variant<E1, E2>> baz() noexcept {
  auto x = foo();
...
std::expected<Z, std::variant<E1, E2>> baz_p2561() {
...
#+end_src
can instead be written using exceptions, with equivalent or better performance:
#+begin_src
X foo() throw(E1);
Y bar() throw(E2);

Z baz() throw(E1, E2) {
  return Z{foo(), bar()}; // aggregate initialization
}
...
environments which were traditionally unable to use exceptions.
