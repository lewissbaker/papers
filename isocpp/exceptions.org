...
calls to ~std::terminate()~ if the program fails to handle an unexpected exception.
...
subset of the set of exception types listed in the exception-specification then the
