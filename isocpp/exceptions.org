...
Adds ~throw()~ specifiers which can be used to declare a function as having either
a static exception specifier or dynamic exception specifier.

Adds ~declthrow(expr)~ syntax for querying what set of exception types might be thrown
from a particular exception.

Defines rules for statically checking that the bodies of function definitions with
~throw()~ specifiers do not violate their exception specification.
...
For example, the following functions all have the same type:
#+BEGIN_SRC c++
void f() throw(E1, E2);
void g() throw(E2, E1);
...
For example: Using the pack expansion syntax from P1858R2
#+begin_src c++
  ...
  struct _compute_foo_throw_types {
    using ...types = type_list<std::any_exception>...;
  };
...
Here, the function ~foo~ has an exception specification that is either ~throw(FooError, std::bad_alloc)~ or ~throw(...)~.
...
the function may emit instances of ~std::exception~ (e.g. as if via ~throw std::exception{};~)
and does not emit any other type of exception.
...
recompilation of all callers of that function. This is no different to changing the return-type of a
function. e.g. when adding a new entry to a ~std::variant~-returning function.
...
The throw-specification of a function need only be computed when the function is selected by
overload resolution, so that the body of the function is not needlessly instantiated.
...
worth the extra complexity/specification effort. In the author's opinion, it is probably not necessary
to add in the initial version. It can be added later if usage experience shows that it would have
sufficient value.
...
contains a mixture of sub-expressions with a static-exception-specification and sub-expressions
with a dynamic-exception-specification (i.e. ~throw(...)~)
...
This paper proposes the latter, for the following reasons:
- It can be used to determine what types might be instantiated with a ~template catch~
  block (see section on this below) that contains this expression.
- It can be used to allow handling of at least some static types statically

The argument for the latter is that it at least lets libraries know what type of static
exceptions might be thrown and thus what types a ~template catch~ (see below) handler
will be instantiated with. It also allows at least some types of exceptions to be handled
statically, without resorting to ~catch(...)~ and ~std::current_exception()~.
