...
- [[https://wg21.link/N3227][N3227]] - Please reconsider ~noexcept~
...
in the return-type for SFINAE purposes.

Since this paper was written, we have gained support for contracts in C++20

**** 

With the (re)introduction of throw-specifications, the task of computing a correct throw-specification
from a set of sub-expressions becomes even more onerous, as you need to compute lists of types,
...
function from the definition of the body of the function.
...
not possible to cast every function to a function-pointer of the same type and then cast that function-pointer
to a function-pointer with a ~noexcept(false)~ exception-specification.
...
While in some cases we could define the language such that the compiler could deduce
the exception-specification for recursive functions by analysing the expressions which
are not recursive calls, it is relatively easy to construct examples where such
rules.

Consider:
#+begin_src c++
  void confounding(int arg) throw(auto) {
    if constexpr (noexcept(confounding(arg)) {
        throw X{};
    } else {
        return confounding(arg - 1);
    }
  }
#+end_src

Querying the exception specification from within the definition of the function is ill-formed.

TODO: Expand on this more.
- cases that might work
- mutually recursive functions with deduced specifications
