...
- Adding the ability to add ~throw()~ specifiers to function declarations as a way of declaring
  the complete set of exception types that the function may exit with.
...
dynamic exception specification to instead deduce to the type `std::exception_ptr`.
