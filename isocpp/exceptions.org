...
This can be especially problematic for services that use exceptions for error-handling which
...
expensive then this can easily result in a snow-balling effect where a few small
...
to throw types derived from those types. See [[id:c9746604-a7f4-4ccc-926a-52edb899d492][(Re)Adding ~throw()~ specifiers]] for more
details.
...
or dynamic exception specifier, ~throw(...)~ or ~throw(std::any_exception)~.
...
at the time that a ~contract_assert()~ statement is compiled.
