...
#+TITLE: P3166R0: Static Exception Specifications
#+AUTHOR: Lewis Baker
#+EMAIL: lewissbaker@gmail.com
#+DATE: 2024-03-16
#+OPTIONS: html5-fancy
#+OPTIONS: tasks:nil
#+EXPORT_FILE_NAME: P3166R0
...
like ~throws Exception~, or ends up catching and ignoring the exception via a ~catch~ with an empty
handler.

Note that the status quo in C++ is that any function not marked ~noexcept~ has an exception specification
that is the equivalent of Java's ~throws Exception~, so if dynamic exceptions are suitable for your
...
- Use of ~throw(auto)~ could be used on functions that compose multiple systems to allow
  the compiler to compute the exception-specification - with the tradeoff that these functions
  must have their definitions visible to users. Alternatively, use of ~throw~ specfications
  using ~declthrow()~ queries could be used in the declaration if separate compilation is
  required.
- Multiple subsystems could throw types like ~std::error_code~ to represent error conditions
  so that there is overlap between sets of potentially-thrown exceptions from different
  subsystems and the errors.
...
Joe Duffy has written an excellent [[https://joeduffyblog.com/2015/11/03/blogging-about-midori/][blog series]]
...
[[https://joeduffyblog.com/2016/02/07/the-error-model/][Midori Error Model]]
...
- [[https://wg21.link/N3202][N3202]] - To which extent can noexcept be deduced (Stroustrop) (2010)
  - Summarises [[https://wg21.link/N3227][N3227]] "Please reconsider ~noexcept~" (2010)
...
- [[https://wg21.link/N3207][N3207]] - noexcept(auto) (2010)
  - Highlights issues with implicit deduction in N3202
    ...
    - Issues with eager function template instantiation to determine function signature/noexceptness
      (not SFINAE friendly).
  - Proposes ~noexcept(auto)~ as in-between compromise between having to duplicate body in noexcept declaration,
    and fully implicit, which has issues.
...
- [[https://wg21.link/N3227][N3227]] - Please reconsider noexcept (2010)
...
- [[https://wg21.link/N3204][N3204]] -  Deducing "noexcept" for destructors (2010)
  - Short paper
  - Wording for default exception-specification user-defined destructors to be the same as for an implicit destructor.
...
- [[https://wg21.link/N4473][N4473]] - ~noexcept(auto)~, again (2015) (Voutilainen)
...
- [[https://wg21.link/P0133R0][P0133R0]] -  Putting ~noexcept(auto)~ on hold, again (2015) (Voutilainen)
...
- P0709 Zero-overhead deterministic exceptions (Herb Sutter)
  ...
  - try expression / statement
    - Require every potentially throwing expressions/statement inside a ~throws~ function
      to be covered by a ~try~ expression.
...
- whether to consider all identifier labels as reachable or whether to only consider an identifier
  label reachable if there is a corresponding ~goto~ statement that targets the label that is also
  reachable.
- whether to consider sub-expressions that are sequenced after other interrupted-flow expressions
  ...
  thrown exceptions.
  For example: What should the deduced exception specification of ~f()~ and ~g()~ be?
  #+begin_src c++    
...
or ~std::bad_cast~. However, doing so is a potential breaking change for any implementations
...
however, since the compiler needs to generate code to throw ~std::bad_cast~ for ~dynamic_cast~
expressions.
...
cause existing code to become ill-formed if ~catch(std::exception_ptr)~ does not
...
was being rethrown by an async task library that it was using ~std::rethrow_exception(eptr)~
...
propagate the exception. Upon further investigation, I found an issue[fn:ice] that indicated
...
an alternative API for obtaining the current exception an an ~exception_ptr~ and also
...
the ~noexcept~ qualifier to all headers included by "the_c_api.h", not necessarily
...
Otherwise, adding a ~contract_assert~ might change the signature of a function with a deduced
...
calling ~std::unreachable()~ does not throw an exception, and be equivalent to
~std::terminate()~ in modes where it does throw an exception.
...
specifications without having to write ~catch(...)~ in any handlers.
...
to have this apply that exception specification to its ~operator()~, and also to require
that the wrapped callable has a compatible exception specification.

**** ~std::reference_wrapper~

This type could have its ~operator()~ modified to have a deduced exception specification
so that if the referenced object has an ~operator()~ with a static exception specification
then the ~reference_wrapper::operator()~ also has an equivalent exception specification.
...
At the moment, algorithms tend to have ~noexcept(false)~ exception specifications and so would
require a ~try/catch(...)~ around calls to them if used inside a function with a static exception
specification.
