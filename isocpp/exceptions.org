...
- Enable static checking of exception specifications for function definitions with
  a ~throw()~ specifier to ensure that they can not exit with exceptions not listed
  in their throw specification.

The goal is to eliminate the performance-related reasons for not using exceptions
in embedded and other performance critical code, and to also improve the ability
to identify at compile-time, code that is failing to handle some exception-types
...
safety-critical systems. Such hidden calls to ~std::terminate()~ might only be
discovered as reachable at runtime.
...
This design is more general than the design proposed in [[https://wg21.link/P0709R4][P0709R4 - Zero-overhead deterministic exceptions: Throwing values]]
...
The paper [[https://wg21.link/P0709][P0709]] by Herb Sutter contains some detailed motivation for improving exceptions,
and does a better job than I could of covering the background. Much of the
...
- Exceptions are expensive on many platforms for error-handling that is not rare.
  ...
  - The alternatives can incur runtime overhead
    - The result is type-erased and requires branching to the code-path to handle
      each of the cases.
...
- This paper explores language changes that would allow exceptions to be several
  orders of magnitude faster - performance of the order of that of a normal return-value.
...
# TODO: Add example of mutually recursive functions here.
...
TODO: Insert link to design discussion.
...
- If execution flows off the end of a handler of a /function-try-block/ then the
  exception is implicitly rethrown as if there was a ~throw;~ statement inserted at
  the end of the handler's /compound-statement/.

Therefore, we need to first define some rules around defining the reachability of certain
statements. These rules will need to be somewhat conservative as doing an accurate computation
of reachability is equivalent to solving the halting problem, and thus intractable.
...
TODO: Need to add a rule for handling do-while statement where the body is an /interrupted-flow-statement/ that
is not a ~break;~ or ~continue;~.
#+begin_src c++
  do { throw X{}; } while(false);         // interrupted-flow statement
  do { std::terminate(); } while (false); // interrupted-flow statement
  do { break; } while(false);             // not an interrupted-flow-statement
  do { continue; } while(false);          // not an interrupted-flow-statement
#+end_src
...
- A /conditional-expression/ where either;
  - the first sub-expression is an /interrupted-flow-expression/; or
  - both the second and third sub-expressions are both /interrupted-flow-expressions/.
- A built-in logical AND or logical OR expression where the first sub-expression is an /interrupted-flow expression/.
- A prvalue expression of type whose destructor is marked ~[[noreturn]]~.
- Or any other compound expression that has a sub-expression that is an /interrupted-flow-expression/.
...
- both;
  - the last sub-statement of the /compound-statement/ is reachable and is not an /interrupted-flow-statement/
  ...
  - There are no object declarations declared in the scope of the /compound-statement/ that
    have destructors that are marked ~[[noreturn]]~.

TODO: Add lots of examples showing various cases here.

**** Reachability of labels

These rules treat all labels as potentially-reachable and does not do any analysis to determine
whether there is any jump-statement that could potentially jump to that label.
...
And even then, a more sophisticated approach should probably then consider the potential
...
the control-flow graph of the entire function in memory and then walking that graph,
marking statements as reachable or not.
This may be incompatible with the architecture of some implementations.

The proposed design chooses a more conservative algorithm that treats all labels as reachable
in order to permit implementations that can compute reachability in a single pass.
...
For example: The current rules result in the following
#+begin_src c++
  void example(bool x) {
    ...
    // Therefore considers the second switch as potentially reachable.
...
switch expression is covered by a case label is non-trivial.
...
Note: If the function is a coroutine and ~return_void~ is found in the scope of
...
is potentially reachable, then the computation of /A/ will take into account
any potentially-thrown exceptions that may result from the evaluation of the
~co_return;~ statement.

If the function is a constructor, then let /B/ be the set of potentially-thrown
...
the set of potentially-thrown exception types for each of the /potentially-reachable statement/s in the
...
There is also the question of whether we should consider the set of potentially-thrown
exceptions of the true or false sub-statements if either the /init-statement/ is a
/interrupted-flow statement/, or if the /condition/ is an /interrupted-flow expression/.

For example: Should the following function deduce to a throw-specification of ~throw(X)~ or
to ~throw(X, Y, Z)~?
#+begin_src c++
  void f(bool cond) throw(auto) {
    if ( (throw X{}, cond) ) {
      throw Y{};
    } else {
      throw Z{};
...
The rules above do not try to compute the individual reachability of the branches and
would result in a deduced exception specification of ~throw(X, Y, Z)~.
...
during constant-evaluation in future - see [[https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2024/p3068r0.pdf][P3068R0]] "Allowing exception throwing in constant-evaluation"
for such a proposal.
...
Q: For ~do~ statements, if the /statement/ is an /interrupted-flow statement/, and thus the
/expression/ is an unreachable statement, should we still consider the set of potentially-thrown
exception-types from the /expression/?
...
as a manifestly constant expression, and therefore cannot exit with an exception.
...
compute their latest-possible point of destruction so that we know to allocate the automatic
...
A ~throw;~ expression that is not a /lexically associated rethrow/ always throws an
...
TODO: Come up with semantics that can handle the following cases:

Given the following exception object and function ~f()~ that might throw such an exception:
#+begin_src c++
  struct X {
...
Basically, any ~dynamic_cast~ expression to reference-type that could fail has
a set of potentially thrown exception types equal to the set { ~std::bad_cast~ }.

A ~dynamic_cast~ expression that is a cast to pointer-type or that would not fail
(e.g. because it was a cast to an pointer to an unambiguous base-class of the operand).

**** ~typeid~

TODO: Describe the kinds of typeid() expressions that might result in an exception.
